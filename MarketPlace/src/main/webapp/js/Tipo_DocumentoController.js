...
/* Controllers */
var module = angular.module('adminTipo_Documento.controllers', []);

module.controller('Tipo_DocumentoCtrl', ['$scope', '$filter', '$http', function ($scope, $filter, $http) {
	//listar
	$scope.lista = {};
    ...
    $scope.guardar = function () {
        ...
        if ($scope.nuevoRegistro) {
            ...
            $http.post('./webresources/Tipo_Documento', JSON.stringify($scope.datosFormulario), {}
            ).success(function (data, status, headers, config) {
            ...
            $http.put('./webresources/Tipo_Documento', JSON.stringify($scope.datosFormulario), {}
            ).success(function (data, status, headers, config) {
                alert("Los datos han sido guardados con Exito");
                $scope.panelEditar = false;
                $scope.listar();
	...
	$scope.eliminar = function (data){
		$http.delete('./webresources/Tipo_Documento/'+data.id, {})
		    .success(function (data, status, headers, config) {
		        $scope.listar();
		    }).error(function (data, status, headers, config) {
